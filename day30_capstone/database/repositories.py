...
class UserRepository:
    ...
    def get_user_history(self, user_id: int) -> List[models.Interaction]:
        return (
            self.db.query(models.Interaction)
...
class ContentRepository:
    ...
    def get_content_by_skills(self, skill_ids: List[int]) -> List[models.Content]:
        ...
        return (
            self.db.query(models.Content)
            .join(models.ContentSkill)
            .filter(models.ContentSkill.skill_id.in_(skill_ids))
            .all()
        )

    def create_content(
        self, title: str, category: str, difficulty: str, popularity: float = 0.0
    ) -> models.Content:
