...
class RecommendationOrchestrator:
    ...
    def get_recommendations(
        ...
        user_history, user_similarities, item_similarities, all_content = (
        ...
        if is_cold_start:
            ...
            if strategy == "collaborative":
                candidates = generator.collaborative_candidates(
                    user_id, limit=limit * 3
                )
            elif strategy == "content":
                candidates = generator.content_based_candidates(
                    user_id, limit=limit * 3
                )
    ...
    def record_feedback(
        self, user_id: int, content_id: int, interaction_type: str, rating: float = None
    ):
        ...
        keys_to_delete = [
            k for k in self.cache.keys() if k.startswith(f"recs_{user_id}_")
        ]
