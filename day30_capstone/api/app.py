...
API_KEY = os.getenv("API_KEY", "capstone-auth-key-2026")
...
async def add_process_time_and_tracing(request: Request, call_next):
    ...
    app.state.metrics["average_latency_ms"] = (
        (current_avg * (n - 1)) + duration_ms
    ) / n
...
def get_recommendations(
    ...
    try:
        ...
        recs = orchestrator.get_recommendations(
            user_id, limit=limit, strategy=strategy, refresh=refresh
        )
