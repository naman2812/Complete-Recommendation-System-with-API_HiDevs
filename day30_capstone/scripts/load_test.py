...
def simulate_user(user_id):
    ...
    try:
        response = requests.get(f"{API_URL}/recommend/{user_id}?limit=5", headers=HEADERS)
...
def run_load_test(num_concurrent_users=10):
    ...
    try:
        ...
        print("Error: API is not running. Please start the API server first (uvicorn api.app:app).")
        return
